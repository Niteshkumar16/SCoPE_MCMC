...
int throwDice(Task chain, Task *next, MultiGaussian *MG, int scale)
{
  ...
  // 2. Keep FAST (nuisance) parameters fixed (they will be dragged by the slave)
  for (unsigned int i = NUM_SLOW; i < PARAMETERS; i++)
  {
    next->f[i] = chain.f[i]; 
    next->f[RANDPOS + i] = 0.0;
  }

  // 3. STASH: Store the OLD slow parameters in the unused space of the task array 
  // (Index 100 is safe since MAX_TASKARRAY_SIZE is 150)
  for (unsigned int i = 0; i < NUM_SLOW; i++)
  {
    next->f[100 + i] = chain.f[i]; 
  }
...
int myslave_rank(int ii) { return ii; }
int middleman_slave_rank(int ii, int rank) { return rank; }
int middleman(int rank) { return 1; } // Every slave acts as its own middleman
int mymiddlemann(int rank) { return rank; } // Slaves report directly to themselves
// ----------------------------------------------------------
...
unsigned int chainSize[CHAINS];           
  unsigned int chainTotalPerformed[CHAINS]; 
  unsigned int chainBack[CHAINS] = {0};     
  unsigned int checkSize = 0;               
  
  // FIX: Allocate as a contiguous 2D array pointer so sizeof() and memory maps work perfectly
  Task (*chain)[MAXCHAINLENGTH] = malloc(CHAINS * sizeof(*chain));
  
  if (chain == NULL) {
      printf("CRITICAL ERROR: Failed to allocate memory for chain!\n");
...
int slave(int rank) 
{
  ...
  // Allocate Cl arrays on the heap ONCE, OUTSIDE THE LOOP to prevent memory leaks
  ...
  double *Old_Cl_TT = (double*)malloc((l_max_alloc + 1) * sizeof(double));
  double *Old_Cl_TE = (double*)malloc((l_max_alloc + 1) * sizeof(double));
  double *Old_Cl_EE = (double*)malloc((l_max_alloc + 1) * sizeof(double));
  double *Old_Cl_BB = (double*)malloc((l_max_alloc + 1) * sizeof(double));
  ...
  double lbound[MAX_TASKARRAY_SIZE];
  double hbound[MAX_TASKARRAY_SIZE];
  double dummy_sig[MAX_TASKARRAY_SIZE];
  get_parameter_bounds_from_config(lbound, hbound, dummy_sig);
  ...
  for (;;) 
    {  
      ...
      // --- RECONSTRUCT OLD TASK ---
      double task_old[MAX_TASKARRAY_SIZE];
      for(int j = 0; j < MAX_TASKARRAY_SIZE; j++) task_old[j] = task[j];
      for(int j = 0; j < 6; j++) {
          // Prevent overwriting valid init params with 0.0 on the very first step
          if (task[100 + j] != 0.0) {
              task_old[j] = task[100 + j]; 
          }
      }

      // --- 1. RUN CAMB (SLOW STEP) TWICE ---
      int magic_old = param_iface(rank, task_old, Old_Cl_TT, Old_Cl_TE, Old_Cl_EE, Old_Cl_BB);
...
// --- 2. THE DRAGGING LOOP (FAST STEP) ---
          for(int i = 1; i < n_steps; i++) {
              ...
              double lnL_old_prop = -0.5 * run_plc(rank, task_old, Old_Cl_TT, Old_Cl_TE, Old_Cl_EE, Old_Cl_BB);
              double lnL_new_prop = -0.5 * run_plc(rank, task, Cl_TT, Cl_TE, Cl_EE, Cl_BB);

              // Calculate Interpolating Distributions
              double P_int_curr = ((n_steps - i) * lnL_old_curr + i * lnL_new_curr) / n_steps;
              double P_int_prop = ((n_steps - i) * lnL_old_prop + i * lnL_new_prop) / n_steps;

              // Metropolis acceptance for the fast step
              if (log(posRnd(1.0)) < (P_int_prop - P_int_curr)) {
      ...
      Chi2X = -Chi2X; // Re-apply the negative sign in our existing MCMC logic uses
      
      // (Memory is no longer freed here, it is freed at the very end of the function)
    ...
    // Free memory when loop exits (it's infinite, but good practice)
    free(Cl_TT); free(Cl_TE); free(Cl_EE); free(Cl_BB);
    free(Old_Cl_TT); free(Old_Cl_TE); free(Old_Cl_EE); free(Old_Cl_BB);
...
void setVariables() {
    ...
    // --- NEW FIX: Ensure the array is large enough to send our stash at index 100 ---
    if (TASKARRAY_SIZE < 110) {
        TASKARRAY_SIZE = 116;
    }
    // --------------------------------------------------------------------------------
