...
! w_lam of no use, cs2_lam now is ce^2
!         integer, parameter :: w_w0wa = 1, w_file = 2
!         integer w_method
         logical usew0wa
         ...
         if(usew0wa) then
            grho_de=grhov*a**(1._dl-3.*w0_ppf-3.*wa_ppf)*exp(-3.*wa_ppf*(1._dl-a)) 
         else
            if(a.eq.0.d0)then
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	use MassiveNu
        ...
        real(dl) c_Gamma_ppf 
        public f_G_ppf, f_zeta_ppf, g_ppf, gdot_ppf, c_Gamma_ppf, setcgammappf
        ...
        function f_zeta_ppf(a)
        real(dl) :: f_zeta_ppf
        real(dl), intent(IN) :: a
 
        f_zeta_ppf=0._dl 
 
        end function f_zeta_ppf

        function gdot_ppf(a,k)
        real(dl) :: gdot_ppf
        real(dl), intent(IN) :: a,k
        
        gdot_ppf=0._dl 

        end function gdot_ppf
...
!Gauge-dependent perturbation equations
    ...
    !Tensor vars
            ...
            real(dl) dgrho_e_ppf, dgq_e_ppf, dgpi_e_ppf
            real(dl) dgrhoec_ppf, dgqec_ppf, vTc_ppf, accu_ppf, delcc_test !test       
...
!precalculated arrays
       ...
       contains
        ...
        subroutine CopyScalarVariableArray(y,yout, EV, EVout)
          ...
          !     yout(EVout%w_ix+1)=y(EV%w_ix+1)  !ppf
          !end if  
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        real(dl) ISW
        real(dl) dgrhoe,dgqe,dgpie,hdotoh,ppiedot,w_eff
...
!  have to get z from eta for numerical stability       
        ...
        ppiedot=3._dl*dgrhoe+dgqe*(12._dl/k*adotoa+k/adotoa-3._dl/k*(adotoa+hdotoh))+ &
                grhov_t*(1+w_eff)*k*z/adotoa+2._dl*EV%Kf(1)*dgpie &
                -2._dl*k2*EV%Kf(1)*(yprime(EV%w_ix)/adotoa-2._dl*y(EV%w_ix))
        ...
        diff_rhopi = pidot_sum - (4*(dgpi-dgpie)+ dgpi_diff )*adotoa + ppiedot !!test
...
!        if (w_lam /= -1 .and. w_Perturb) then
         y(EV%w_ix) = InitVec(i_clxq)   !ppf: Gamma=0 
...
!  Internal variables.
	  ...
	  real(dl) grhoT,dgrhoT,dgqT,gpresT,vT,dgpiT,Gamma,Phi_minus,ppiTdot, &
                 S_Gamma,ckH,Gammadot,Fa,dgqe,dgrhoe
	  real(dl) gppf,gdotppf,fGppf,fzetappf,gammacond !test
        real(dl) w_eff
        real(dl) grhormass_t,rhonu,pnu,grhonu_t,gpnu_t,pinu,pinudot,shearnu, &
                 shearnudot,rhonudot 
...
!  Get sound speed and ionisation fraction.
        ...
        grhoT=grhob_t+grhoc_t+grhor_t+grhog_t
        gpresT=grhor_t/3._dl+grhog_t/3._dl
...
!  8*pi*a*a*SUM[rho_i*clx_i]
        dgrhoT=grhob_t*clxb+grhoc_t*clxc 
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        dgqT=grhob_t*vb
                
        if (CP%Num_Nu_Massive > 0) then
           call MassiveNuVars(EV,ay,a,grhoT,gpresT,dgrhoT,dgqT, wnu_arr)
        end if
...
!       end if
       ...
       if (EV%no_nu_multpoles) then
        ...
        z=(0.5_dl*dgrhoT/k + etak)/adotoa  !NOTE: neglect dgrhoe here 
        dz= -adotoa*z - 0.5_dl*dgrhoT/k    !NOTE: neglect dgrhoe here
        clxr=-4*dz/k
        qr=-4._dl/3*z
        pir=0
        pirdot=0  
...
!  Photons
        ...
        if (.not. EV%TightCoupling) then 
         ...
         pig=0    !NOTE: pig=0 if tight coupling
        endif
       end if

!  8*pi*a*a*SUM[rho_i*clx_i] - radiation terms
        dgrhoT=dgrhoT + grhog_t*clxg+grhor_t*clxr 
       
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        dgqT=dgqT + grhog_t*qg+grhor_t*qr
        vT=dgqT/(grhoT+gpresT)
! 8*pi*a*a*SUM[(rho_i+p_i)*sigma_i]*3/2
        dgpiT=grhor_t*pir+grhog_t*pig    !NOTE: for now, no massive nu
...
!  Photon mass density over baryon mass density
        ...
        Gamma=ay(EV%w_ix)
        Phi_minus=(dgrhoT + 3*adotoa/k*dgqT + EV%kf(1)*dgpiT)/2._dl/k2/EV%kf(1) - Gamma
...
!        end if
 	  gppf = g_ppf(a,k)
	  gdotppf = gdot_ppf(a,k)
	  fGppf = f_G_ppf(a)
	  fzetappf = f_zeta_ppf(a)

	  sigma = (etak + (dgrhoT + 3*adotoa/k*dgqT)/2._dl/k)/EV%kf(1) - k*Gamma & 
                + k*gppf*Phi_minus
	  sigma = sigma/adotoa
        ...
        if (.not. EV%no_phot_multpoles) then
           if (.not. EV%tightcoupling) then
             ...
             pigdot=0   !NOTE: pigdot=o if tight coupling
           end if
         end if
         ppiTdot=grhor_t*(pirdot-4*adotoa*pir)+grhog_t*(pigdot-4*adotoa*pig)  !for now, no massive nu

	  S_Gamma=gppf*(dgpiT + ppiTdot/adotoa)
	  S_Gamma=S_Gamma-((gppf+fzetappf+gppf*fzetappf)*(grhoT+gpresT) &
          -grhov_t*(1+w_eff))*(vT+sigma)*k/adotoa
	  S_Gamma=S_Gamma/2._dl/k2/(1+gppf)+ &
          (gdotppf/adotoa-2*gppf)*Phi_minus/(1+gppf)
...
! for test
	  gammacond=abs(Gamma/Phi_minus - fGppf)
          if(ckH*ckH.gt.3.d1.and.gammacond.lt.1d-5)then
	        Gamma=fGppf*Phi_minus
                Gammadot=0.d0
	        ayprime(EV%w_ix)=Gammadot
	  endif
	  Fa=1+3*(gppf+1)*(grhoT+gpresT)/2._dl/k2/EV%kf(1)
	  dgqe=S_Gamma - Gammadot/adotoa - Gamma + fzetappf*(grhoT+gpresT) &
            *(vT+sigma)/2._dl/k/adotoa
  	  dgqe=-dgqe*(gppf+1)/Fa*2._dl*k*adotoa + vT*grhov_t*(1+w_eff)
	  dgrhoe=-2*k2*EV%kf(1)*Gamma+2*gppf*Phi_minus*k2*EV%kf(1)-3/k*adotoa*dgqe
	  dgrho=dgrhoT+dgrhoe
	  dgq=dgqT+dgqe
        z=(0.5_dl*dgrho/k + etak)/adotoa 
	  ...
	  EV%dgq_e_ppf=dgqe
	  EV%dgpi_e_ppf=-2._dl*gppf*Phi_minus*k2  
          EV%accu_ppf=Phi_minus  !abs(sigma-(z+1.5_dl*dgq/k2)/EV%kf(1))/max(abs(sigma),1.d-30) ! of no use
          EV%dgrhoec_ppf=dgrhoe+3._dl*(1+w_eff)*grhov_t*adotoa*vT/k
          EV%dgqec_ppf=dgqe+(1+w_eff)*grhov_t*sigma
          EV%vTc_ppf=vT+sigma
          EV%delcc_test=clxc+3._dl*adotoa*vT/k
        sigma=(z+1.5_dl*dgq/k2)/EV%kf(1) !test
...
!  Use explicit equation for vb if appropriate
    ...
    !  First-order approximation to baryon-photon splip
            ...
            !AL: First order slip seems to be fine here to 2e-4
             ...
             dgs = grhog_t*pig+grhor_t*pir + EV%dgpi_e_ppf  ! no massive nu?
...
! Also calculate gpres: 8*pi*p*a**2
        ...
        grhor_t=grhornomass/a2
        grhog_t=grhog/a2
        grhov_t=grho_de(a)/a2  !grhov*a**(-1-3*w_lam)
	w_eff=w_de(a)
