...
!     in one set of parameters and produdes the corresponding output. 
    ...
    program driver
        ...
        character(LEN=Ini_max_string_len) numstr, VectorFileName, &
            ...
            LensedTotFileName, LensPotentialFileName, wafile
        integer i
...
!  Read initial parameters.
       ...
       usew0wa = Ini_Read_Logical('usew0wa',.true.)
       if(usew0wa)then
          w0_ppf = Ini_Read_Double('w0', -1.d0)
          wa_ppf = Ini_Read_Double('wa', 0.d0)
          write(*,'("(w0, wa) = (", f8.5,", " f8.5, ")")')w0_ppf,wa_ppf
       else 
          wafile = Ini_Read_String('wafile')    
